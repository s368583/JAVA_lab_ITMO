...
public class Shorty extends Person {
    ...
    public boolean run (){
        ...
        if (run) {
            ...
            City city = (getLocation()).getAddress().getCity();
            city.sound("свобода");
            Street street = null;
            do {
                street =  (((Street)(this.getLocation())).getAddress().getCity().getStreets().get((int)(Math.floor(Math.random()*((Street)(this.getLocation())).getAddress().getCity().getStreets().size()))));
            }
    ...
    public void court (Person person2){
        new Judgment(){
            @Override
            public void process() {
                System.out.println(getName() + " подал в суд на " + person2);
                if (Math.random() > 0.5){
                    System.out.println("Мир жесток. " + person2 + " посадили в тюрьму");
                    person2.move(person2.getLocation().getAddress().getCity().getJail());
                }
                else {
                    System.out.println(person2 + " выиграл дело!");
                }
            }
        }.process();
    }
